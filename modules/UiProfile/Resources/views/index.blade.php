		...
		<div class="panel panel-visible">
            <div class="panel-heading">
                <div class="panel-title hidden-xs">
                    <span class="glyphicon glyphicon-tasks"></span>Lista de perfis de acesso
                ...
                <table class="table table-striped table-bordered table-hover" id="datatableUser" cellspacing="0" width="100%">
                    ...
                    <tbody>
                        @foreach($ui_profiles as $ui_profile)
                            <tr>
                                <td>{{ $ui_profile->id }}</td>
                                <td>{{ $ui_profile->name }}</td>
                                <td>@if($ui_profile->root) {{ 'Sim' }} @else {{ 'Não' }} @endif</td>
                                <td>
                                    <a href="{{ url($module_link .'/permission/'. $ui_profile->id) }}" class="btn btn-xs btn-primary">
                                        <i class="fa fa-check-square">&nbsp;&nbsp;Permissões</i>
                                    </a>
                                    &nbsp;
                                    <a href="{{ url($module_link .'/edit/'. $ui_profile->id) }}" class="btn btn-xs btn-warning">
                                    ...
                                    @else
                                        <a href="{{ url($module_link .'/delete/'. $ui_profile->id) }}" class="btn btn-xs btn-danger">
                                            <i class="fa fa-minus-square">&nbsp;&nbsp;Remover</i>
                                        </a>
                                    @endif
                                </td>
	...
	jQuery(document).ready(function() {
		$('#datatableUser').dataTable({
            "columns": [
                ...
                null,
                { "width": "8%", "className": "center" },
                { "width": "28%", "className": "center", "orderable": false },
            ],
            "oLanguage": {
                "oPaginate": {
                    "sPrevious": "",
